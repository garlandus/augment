    ...
    (as: Seq[A], bs: Seq[B], cs: Seq[C], f: (A,B,C) => X, g: X => Z) =>
      val m = as.map:
        a => bs.map:
          b => cs.map:
            c => g(f(a, b, c))
      MultiArrayC (as, bs, cs, m, Array[Z]())
...
given ComprehensionD[SeqD] with
  def irregular[X, Z, A, B, C, D]: IrregComprehensionD[SeqD, X, Z, A, B, C, D] =
    (as: Seq[A], bsDep: DepSeqB[A, B], csDep: DepSeqC[A,B,C], dsDep: DepSeqD[A,B,C,D],
        f: (A,B,C,D) => X, g: X => Z) =>
      as.flatMap:
        a => bsDep(a).flatMap:
          b => csDep(a, b).flatMap:
    ...
    (as: Seq[A], bsDep: DepSeqB[A, B], csDep: DepSeqC[A,B,C], dsDep: DepSeqD[A,B,C,D],
      ...
      as.map:
        a => (a, bsDep(a).map(
          b => (b, csDep(a, b).map(
            c => (c, dsDep(a, b, c).map(
              d => (d, g(f(a, b, c, d)))).toMap)).toMap)).toMap)
      .toMap

given ComprehensionD[MultiArrayD] with
  def irregular[X, Z, A, B, C, D]: IrregComprehensionD[MultiArrayD, X, Z, A, B, C, D] = ???
  override def rectangular[X, Z, A, B, C, D](using ClassTag[Z])
    : RectComprehensionD[MultiArrayD, X, Z, A, B, C, D] =
    (as: Seq[A], bs: Seq[B], cs: Seq[C], ds: Seq[D], f: (A,B,C,D) => X, g: X => Z) =>
      val m =
        as.map:
          a => bs.map:
...
given ComprehensionE[SeqE] with
  def irregular[X, Z, A, B, C, D, E]: IrregComprehensionE[SeqE, X, Z, A, B, C, D, E] =
    (as: Seq[A], bsDep: DepSeqB[A, B], csDep: DepSeqC[A,B,C], dsDep: DepSeqD[A,B,C,D],
        esDep: DepSeqE[A,B,C,D,E], f: (A,B,C,D,E) => X, g: X => Z) =>
      as.flatMap:
        a => bsDep(a).flatMap:
          b => csDep(a, b).flatMap:
            c => dsDep(a, b, c).flatMap:
              d => esDep(a, b, c, d).map:
                e => g(f(a, b, c, d, e))
...
given ComprehensionE[ArrayNE] with
  ...
  override def rectangular[X, Z, A, B, C, D, E](using ClassTag[Z]): RectComprehensionE[ArrayNE, X, Z, A, B, C, D, E] =
    (as: Seq[A], bs: Seq[B], cs: Seq[C], ds: Seq[D], es: Seq[E],
        f: (A,B,C,D,E) => X, g: X => Z) =>
      as.map:
        a => bs.map:
          b => cs.map:
    ...
    (as: Seq[A], bsDep: DepSeqB[A, B], csDep: DepSeqC[A,B,C], dsDep: DepSeqD[A,B,C,D],
      ...
      as.map:
        a => (a, bsDep(a).map(
          b => (b, csDep(a, b).map(
            c => (c, dsDep(a, b, c).map(
              d => (d, esDep(a, b, c, d).map(
                e => (e, g(f(a, b, c, d, e)))).toMap)).toMap)).toMap)).toMap)
      .toMap

given ComprehensionE[MultiArrayE] with
  def irregular[X, Z, A, B, C, D, E]: IrregComprehensionE[MultiArrayE, X, Z, A, B, C, D, E] = ???
  override def rectangular[X, Z, A, B, C, D, E](using ClassTag[Z])
