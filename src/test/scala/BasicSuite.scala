...
class BasicSuite extends munit.FunSuite:
  ...
  val mult = augment(Numeric[Int].times)
  ...
  test("basic B"):
    ...
    def appendAsterisks = (s: String, n: Int) => s + "*" * n
    def g = (sts: Seq[String], ls: Seq[Int]) => sts.mkString(" ") + " " + ls.sum

    val (sts1, sts2) = (List("ab", "cd", "ef"), List("gh", "ij", "kl"))
    augment(appendAsterisks) crossCheck (
      sts1,
      as,
      for
        s <- _
  ...
  test("basic C"):
    ...
    def f = (x: Int, y: Int, s: String) => s"$s: ${x * y}"

    augment(f) crossCheck (
      as,
      bs,
      cs,
      for
  ...
  test("persons"):
    val getNames = (p: Person, c: Person) => (p.name, c.name)

    augment(getNames) crossCheck (
      persons where (!_.isMale),
      _.children,
      (persons
        .filter: p =>
          !p.isMale)
  ...
  test("permutations unrolled"):
    ...
    val y1 = concatS.applySet(s, s - _, s - _ - _, s - _ - _ - _)
  ...
  def sieve(n: Int) = (2 to sqrt(n)) * (2 to n / _)
  def sieve1(n: Int) = (2 to sqrt(n)) * (x => 2 to n / x)
  ...
  def primesB1(n: Int): Seq[Int] =
    complement(primesB1(sqrt(n)) * (2 to n / _), 2 to n) until n == 1

  def primesB2(n: Int): Seq[Int] =
    if n == 1 then Seq()
    else complement(primesB2(sqrt(n)) * (2 to n / _), 2 to n)
  ...
  def primesC1(n: Int): Seq[Int] =
    complement(primesC1(sqrt(n)) * (x => x to n / x), 2 to n) until n == 1

  def primesC2(n: Int): Seq[Int] =
    if n == 1 then Seq()
    else complement(primesC1(sqrt(n)) * (x => x to n / x), 2 to n)
  ...
  test("vectorize"):
    var res1 = mult(Seq((1, 2), (4, 3), (5, 6)))
    assertEquals(res1, Vector(2, 12, 30))
    var res2 = add(Seq((1, 2, 4), (4, 3, 7), (5, 6, 1)))
    assertEquals(res2, Vector(7, 14, 12))

  test("bubble sort"):
    def swap[T](a: Array[T]) = (i: Int, j: Int) => { val t = a(i); a(i) = a(j); a(j) = t }
    def orderArrayPair[T](using Ordering[T]) =
      (a: Array[T]) => ((i: Int, j: Int) => if a(j) > a(j + 1) then swap(a)(j, j + 1))
  ...
  test("Pythagorean triangles revisited"):
    val n = 20
    ...
    assertEquals(trianglesA, trianglesC)
    assertEquals(trianglesA, trianglesD)
    assertEquals(trianglesA, trianglesE)
